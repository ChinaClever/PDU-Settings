...
Dev_MpduCustomizeCfg::Dev_MpduCustomizeCfg(QObject *parent) : Dev_Object(parent)
{
    mDev = mPacket->getMpduCustomize();
    initCfg();
}

Dev_MpduCustomizeCfg *Dev_MpduCustomizeCfg::bulid(QObject *parent)
{
    static Dev_MpduCustomizeCfg* sington = nullptr;
    if(sington == nullptr)
        sington = new Dev_MpduCustomizeCfg(parent);
    return sington;
}

void Dev_MpduCustomizeCfg::initData()
{
...
void Dev_MpduCustomizeCfg::writeData()
{
    sDevType *ptr = &(mDev->dt);
...
void Dev_MpduCustomizeCfg::initCfg()
{
    initData();
}

void Dev_MpduCustomizeCfg::save()
{
    writeData();
}
