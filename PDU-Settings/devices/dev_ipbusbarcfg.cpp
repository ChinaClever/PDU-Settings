...
Dev_IpBusbarCfg::Dev_IpBusbarCfg(QObject *parent) : Dev_Object(parent)
{
    mDev = mPacket->getIpBusbar();
    initCfg();
}

Dev_IpBusbarCfg *Dev_IpBusbarCfg::bulid(QObject *parent)
{
    static Dev_IpBusbarCfg* sington = nullptr;
    if(sington == nullptr)
        sington = new Dev_IpBusbarCfg(parent);
    return sington;
}

void Dev_IpBusbarCfg::initData()
{
...
void Dev_IpBusbarCfg::writeData()
{
    sDevType *ptr = &(mDev->dt);
...
void Dev_IpBusbarCfg::initCfg()
{
    initData();
}

void Dev_IpBusbarCfg::save()
{
    writeData();
}
