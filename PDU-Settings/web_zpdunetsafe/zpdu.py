...
class Zpdu(ZpduWeb):
    ...
    def start_fun(self , sock , dest_ip , dest_port):
        ...
        intRet , message = self.login2(intRet)
        self.sendtoMainapp(message)
        if( intRet == 0 ):
            return
        time.sleep(13)
    ...
    def checkHtml(self):
        try:
            self.driver.find_element_by_xpath('/html').send_keys('thisisunsafe')
        except:
            pass
    ...
    def setDebugHtml(self):
        ...
        try:
            it = self.driver.find_element_by_id('AF')
            if int(cfg['aboard'])==1:
            ...
            for i in range(1,4):
                ...
                showMessage = 'L{0}相输出位'.format(i)
                self.setItById( strId , int(cfg[strCfgs]) , showMessage)
            self.setSelect( 'ee' , int(cfg['standar']))#定位版本类型  #0:标准  1：中性  2：定制
            self.setSelect( 'tmp' , int(cfg['tem']))#定位温湿度 # 0：标准  1：专用
            if(int(cfg['standar']) == 2):
                self.uploadLogo()#上传logo
    ...
    def uploadLogo(self):
        try:
            ...
            if(len(logopath)==0):
                return
            self.driver.find_element_by_xpath('//*[@id="selectLogo"]').send_keys(logopath)#输入图片位置
            time.sleep(2)
            self.driver.find_element_by_xpath('//*[@id="uploadLogo"]').click()#点击上传
            time.sleep(2)
            self.driver.switch_to.alert.accept()#同意弹窗
        except:
            pass
    ...
    def setSetHtml(self):
        ...
        self.setSelect( 'a8' , 0)###
        self.setSelect( 'a9' , int(cfg['breaker']))
        self.execJs('sethw()')
    ...
    def setDelay(self):
        ...
        try:
            ...
            self.driver.switch_to.parenet_frame()
        except:
            print('设置延时错误')
    ...
    def setOpValue(self):
        ...
        try:
            self.driver.refresh()
            time.sleep(3)
            #.driver.find_elements_by_class_name('treeview')[1].click()
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/a').click()#设备设置
            time.sleep(2)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[3]/a').click()#单元
            #self.driver.find_elements_by_class_name('menuItem')[5].click()
            time.sleep(2)
            self.driver.switch_to.frame('iframe2_c')
            if(int(cfg['series']) != 1):
                for i in range(0 , int(cfg['outputs'])):
                    ...
                    if(int(cfg['series']) == 2 or int(cfg['series']) == 4):
                        ...
                        str = 'g{0}'.format(i)
                        self.setItById(str, int(cfg['zpduopcur_{0}_max'.format(i+1)])*0.8 , '输出位{0}上临值'.format(i+1) , False)
                        str = 'h{0}'.format(i)
        ...
        try:
            ...
            for i in range(0 , int(cfg['outputs'])):
                ...
                if(int(cfg['series']) == 2 or int(cfg['series']) == 4):
                    ...
                    str = 'g{0}'.format(i)
                    list.append(str)
                    cfgStr.append(str(int(cfg['zpduopcur_{0}_max'.format(i+1)])*0.8))
    ...
    def setLoopCur(self):
        ...
        try:
            for i in range(4 , 4+int(cfg['loops'])):
                ...
                self.setItById('f{0}'.format(i), int(cfg['loopcur_max'])*0.8 , 'C{0}电流上临界值'.format(i-3) , False)
    ...
    def setLineCur(self):
        ...
        try:
            for i in range(10 , 10+int(cfg['lines'])):
                ...
                self.setItById('f{0}'.format(i), int(cfg['cur_max'])*0.8 , 'L{0}电流上临界值'.format(i-9) , False)
    ...
    def checkSensorValue(self):
        ...
        try:
            ...
            time.sleep(2)
            if(int(cfg['series']) != 1):
                self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[4]/a').click()#环控
            else:
                self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[2]/a').click()#环控
    ...
    def clearEnergy(self):
        ...
        try:
            ...
            #self.driver.find_elements_by_class_name('treeview')[1].click()
            time.sleep(1)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[5]/a').click()
    ...
    def openOrCloseOp(self , flag):
        ...
        try:
            ...
            #self.driver.find_elements_by_class_name('treeview')[1].click()
            time.sleep(1)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[8]/a').click()
            #self.driver.find_elements_by_class_name('menuItem')[10].click()
            time.sleep(1)
    ...
    def checkOp(self , flag):
        ...
        try:
            ...
            time.sleep(1)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[2]/ul/li[8]/a').click()
            #self.driver.find_elements_by_class_name('menuItem')[10].click()#开关
            time.sleep(1)
            self.driver.switch_to.frame('iframe2_i')
            list=[]
            cfgStr = []
    ...
    def setTime(self):
        try:
            self.driver.refresh()
            time.sleep(3)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[3]/a').click()#
            #self.driver.find_elements_by_class_name('treeview')[2].click()#网络安全
            time.sleep(2)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[3]/ul/li[3]/a').click()#
            #self.driver.find_elements_by_class_name('menuItem')[13].click()#ntp
            time.sleep(2)
    ...
    def resetfactory(self):
        try:
            self.driver.refresh()
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[7]/a').click()#
            #self.driver.find_elements_by_class_name('treeview')[6].click()#系统工具
            time.sleep(1)
            self.driver.find_element_by_xpath('//*[@id="sidebar-menu"]/li[7]/ul/li[2]/a').click()
            #self.driver.find_elements_by_class_name('menuItem')[32].click()#系统命令
            time.sleep(1)
