...
class ZpduWeb:
    ...
    def login3(self):
        ...
        try:
            ...
            return 0,'输入IP错误;0'
        ret = self.setItById('name_create', self.cfgs['user'] , '账号')
        if( ret == False ):
            return -1,'没有此控件;1'
        self.setItById('psd_create', self.cfgs['password']  , '密码')
        self.setItById('psd_check', self.cfgs['password']  , '再次输入密码')
